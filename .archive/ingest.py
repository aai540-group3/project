...
class DataIngestionStage(PipelineStage):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.metadata: Dict[str, Any] = {
            "ingestion": {"sources_processed": [], "status": {"successful": [], "failed": []}, "execution_time": None},
    ...
    def _save_raw_data(self, data: pd.DataFrame) -> None:
        ...
        # ... (code to save metadata.json and variables.json) ...

        logger.info(f"Raw data saved to {parquet_path}, {csv_path}, {metadata_path}, and {variables_path}")

        # Log artifacts if tracking is enabled
        self.log_artifact(str(parquet_path))
        self.log_artifact(str(csv_path))
        self.log_artifact(str(metadata_path))
        self.log_artifact(str(variables_path))

        # Upload to S3
        s3_client = boto3.client("s3")
        bucket_name = self.cfg.aws.s3.bucket.name  # Get bucket name from config

        s3_client.upload_file(str(parquet_path), bucket_name, "datasets/data.parquet")
        s3_client.upload_file(str(csv_path), bucket_name, "datasets/data.csv")
        s3_client.upload_file(str(metadata_path), bucket_name, "datasets/metadata.json")
        s3_client.upload_file(str(variables_path), bucket_name, "datasets/variables.json")

    def _save_metadata(self) -> None:
        """Save comprehensive ingestion metadata."""
        output_path = self.get_path("data") / "ingestion_metadata.json"

        # Add summary statistics
        self.metadata["ingestion"]["errors"] = self.ingestion_errors
        self.metadata["ingestion"]["timestamp"] = time.strftime("%Y-%m-%d %H:%M:%S")
