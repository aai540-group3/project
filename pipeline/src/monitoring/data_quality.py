...
class DataQualityMonitor:
    ...
    def check_quality(self, data: pd.DataFrame) -> Dict:
        ...
        metrics = {
            ...
            "drift": self._check_distribution_drift(
    ...
    def _check_duplicates(self, data: pd.DataFrame) -> Dict:
        ...
        return {
            "total": duplicates.sum(),
            "percentage": duplicates.mean() * 100
        }
    ...
    def _check_outliers(self, data: pd.DataFrame) -> Dict:
        ...
        for column in data.select_dtypes(include=[np.number]).columns:
            ...
            IQR = Q3 - Q1
            outliers[column] = {
                "total": ((data[column] < (Q1 - 1.5 * IQR)) |
                         (data[column] > (Q3 + 1.5 * IQR))).sum(),
                "percentage": ((data[column] < (Q1 - 1.5 * IQR)) |
                             (data[column] > (Q3 + 1.5 * IQR))).mean() * 100
            }
        return outliers
