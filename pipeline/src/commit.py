...
def run_command(
    cmd: list[str], capture_stderr: bool = False, verbose: bool = False
...
def untrack_file(file_path: str, verbose: bool = False) -> bool:
    ...
    status, _ = run_command(
        ["git", "commit", "-m", f"Stop tracking {file_path} with Git"], verbose=verbose
    )
    if status != 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Handle DVC commits and Git untracking automatically."
    )
    parser.add_argument(
    ...
    # Track statistics
    stats = {"attempts": 0, "files_untracked": 0, "errors": 0}
    ...
    while True:
        ...
        status, stderr = run_command(
            ["dvc", "commit", "-f"], capture_stderr=True, verbose=args.verbose
        )
        ...
        if not stderr:
            log(
                "DVC commit failed but no error message was captured.",
                verbose=True,
                level="ERROR",
            )
            stats["errors"] += 1
        ...
        if not matches:
            log(
                "DVC commit failed with an unexpected error:",
                verbose=True,
                level="ERROR",
            )
