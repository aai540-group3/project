...
import pandas as pd
from feast import (
    Entity, Feature, FeatureService, FeatureStore, FeatureView,
    FileSource, ValueType
)
...
class DiabetesFeatureStore:
    ...
    def get_online_features(
        ...
        return self.store.get_online_features(
            ...
            features=self._get_all_feature_refs()
        ).to_dict()
    ...
    def _get_all_feature_refs(self) -> List[str]:
        ...
        for view_name, view in self.feature_views.items():
            feature_refs.extend(
                [f"{view_name}:{feat.name}" for feat in view.features]
            )
        return feature_refs
