"""
.. module:: src.models.logistic_regression.evaluate
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        test_data_path = Path(
            to_absolute_path(f"{data_paths.processed}/test.csv")
        )
        model_path = Path(
            to_absolute_path(f"models/{cfg.model.name}/model.pkl")
        )
        metrics_output_path = Path(
            to_absolute_path(
                f"reports/metrics/{cfg.model.name}_metrics.json"
            )
        )
        ...
        with Live() as live:
            try:
                ...
                conf_matrix_path = (
                    metrics_output_path.parent / "confusion_matrix.png"
                )
